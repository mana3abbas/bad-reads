...
updateView = () =>{
  axios.get("http://${process.env.REACT_APP_PRODUCTION_IP}/admin/author")
     .then(res=>{
       const data = res.data;
...
bookpath=()=>{
  ...
  handledeleteauthor=(index)=>{
    axios.delete("http://${process.env.REACT_APP_PRODUCTION_IP}/admin/author/"+this.state.authors[index]._id)
    .then(res=>{
      this.updateView()
      this.props.history.push("/admin/author/")
    }).catch(error=>
