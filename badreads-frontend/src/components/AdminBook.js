...
updateView = () =>{
  axios.get("http://${process.env.REACT_APP_PRODUCTION_IP}/admin/book")
     .then(res=>{       
       const data = res.data;
...
onSubmit = async() => {
  await axios.get("http://${process.env.REACT_APP_PRODUCTION_IP}/admin/author")
  .then(response => {
      var authors=response.data
      axios.get("http://${process.env.REACT_APP_PRODUCTION_IP}/admin/category")
      .then(response => {
...
authorspath=()=>{
  ...
  handledeletebook=(index)=>{
    axios.delete("http://${process.env.REACT_APP_PRODUCTION_IP}/admin/book/"+this.state.books[index]._id)
    .then(res=>{
      this.updateView()
      this.props.history.push("/admin/book/")
    }).catch(error=>
